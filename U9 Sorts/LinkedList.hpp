...
*       code them first: DONE
*
*  ALGORITHM:
*       If there is 0 or 1 node in the list,
*           exit the method (return;) // DONE
*       Find the middle Node (mid) // doone
*       
*       let left = the head pointer of the first partition (the head) // done
*       let right = the head pointer of the second half of the list (to the right of mid) // doone
*       Split the list by setting the mid->next to null // done
*
...
void  LinkedList<Type>::mergeSort(shared_ptr<Node <Type> > &head) {
	if (head == nullptr || head->next == nullptr) { // Nothing to sort
		return;
	}
